@extends('layouts.app')
@section('title', 'Delivery History')
@section('content')
<div class="dashboard-layout">
    <aside class="sidebar">
        <div class="sidebar-brand">LUBOSMART Logistics</div>
        <a href="{{ route('logistics.dashboard') }}" class="sidebar-link">Dashboard</a>
        <a href="{{ route('logistics.applicants') }}" class="sidebar-link">Rider Applicants</a>
        <a href="{{ route('logistics.riders') }}" class="sidebar-link">Manage Riders</a>
        <a href="{{ route('logistics.deliveries') }}" class="sidebar-link">Active Deliveries</a>
        <a href="{{ route('logistics.history') }}" class="sidebar-link active">Delivery History</a>
        <a href="{{ route('logistics.reports') }}" class="sidebar-link">Reports</a>
        <a href="{{ route('logistics.chat') }}" class="sidebar-link">Chat/Messaging</a>
        <a href="{{ route('logistics.account') }}" class="sidebar-link">Account</a>
        <form method="POST" action="{{ route('logistics.logout') }}">
            @csrf
            <button type="submit" class="sidebar-link" style="width:100%; text-align:left; background:none; border:none;">Logout</button>
        </form>
    </aside>

    <main class="main-content">
        <h1 class="page-title">Delivery History</h1>
        <div class="panel">
            <table class="table">
                <thead><tr><th>Order ID</th><th>Rider</th><th>Completed</th><th>Fee</th><th>Status</th></tr></thead>
                <tbody>
                    <tr><td>#VLR-1039</td><td>Rico Domingo</td><td>Sep 2, 2026</td><td>₱75</td><td><span class="badge badge-success">Completed</span></td></tr>
                    <tr><td>#VLR-1035</td><td>Mark Villareal</td><td>Sep 1, 2026</td><td>₱60</td><td><span class="badge badge-success">Completed</span></td></tr>
                    <tr><td>#VLR-1028</td><td>Angelo Reyes</td><td>Aug 30, 2026</td><td>₱50</td><td><span class="badge" style="background:#FEE2E2; color:var(--danger);">Cancelled</span></td></tr>
                </tbody>
            </table>
        </div>
    </main>
</div>
@endsection